...
'''
join_alignments.py v1.1  last modified 2022-07-07

    specify multiple alignments with -a

    species names are expected to be the same, 
      perhaps separated by an optional delimiter with -d
...
def main(argv, wayout):
	...
	parser.add_argument('-P','--long-partition', action="store_true", help="use long RAxML type partition format")
	...
	with open("{}.partition.txt".format(args.supermatrix),'w') as pf:
		for alignfile in alignlist:
			...
			# long format, written like
			# 123, gene1 = 1-123
			if args.long_partition:
				pf.write("{}, {} = {}-{}\n".format( al_length, alignfile , runningsum+1, targetlength ) )
			else:
